...
# Path where Medea writes agy-PID → conversation-ID mappings.
# Each spawned agy process gets a file named by its PID containing our internal ID.
_PID_MAP_DIR = os.path.expanduser("~/.medea/data/pid_map")

def _resolve_medea_conv_id():
    """Walk up the process tree to find which Medea ManagedAgent spawned us.
    
    The hook runs as: medea → agy → [shell?] → python3 broadcast.py
    We walk the PPID chain upward checking for a PID mapping file at each level.
    """
    try:
        pid_chain = []
        current = os.getpid()
        # Walk up to 20 levels (broadcast → shell → agy → medea → init)
        for _ in range(20):
...
def main():
    # Single gate: if this agy session wasn't spawned by Medea, bail immediately.
    medea_id = _resolve_medea_conv_id()
    if not medea_id:
        return
    ...
    try:
        ...
        payload = {
            ...
            "medea_conv_id": medea_id,
            "payload": hook_context
        }

        def _get_dashboard_port():
            port = os.environ.get("MEDEA_PORT")
            if port and port.isdigit():
                return int(port)

            config_path = os.path.expanduser("~/.medea/config.yaml")
