...
f = DiscordFormatter()
clean = f.format_text(content)
print("Total length:", len(clean))
chunks = f.split_message(clean)
...
# Now append footer and test!
stats = f"\n\n*⚡ 1195 tokens · ⏱ 81.76ss*"
clean_with_stats = clean + stats
chunks2 = f.split_message(clean_with_stats)
print("With stats length:", len(clean_with_stats))
for i, chunk in enumerate(chunks2):
    print(f"Chunk2 {i+1} len: {len(chunk)}")
    print(f"--- CHUNK2 {i+1} START ---")
    print(chunk[:100].replace('\n', '\\n'))
    print("...")
    print(chunk[-100:].replace('\n', '\\n'))
    print(f"--- CHUNK2 {i+1} END ---")
