...
class MockMessage:
    ...
    async def edit(self, content):
        self.content = content
        # print(f"Edited message to length {len(content)}")
...
async def main():
    channel = MockChannel()
    streamer = DiscordStreamer(channel, edit_interval=0.0)
    await streamer.start()
    
    # Simulate first edit (without stats)
...
If this structure looks good to you, I can start writing out the exact SQL schema migrations and the Rust structs for forge-medea to implement these models. How does this approach sound?"""
    ...
    metadata = {"tokens": 1195, "duration": 81.76, "model": "Gemini 3.1 Pro (High)"}
    await streamer.finish(metadata)

    # Print final messages
    for i, msg in enumerate(streamer.messages):
        print(f"========== DISCORD MESSAGE {i+1} ==========")
        print(msg.content)
